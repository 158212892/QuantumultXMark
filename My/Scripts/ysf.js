...
------------------------------------------
青龙：
1.捉youhui.95516.com域名包下的Authorization,填写到ysf_data中，多账号换行

Surge、QuantumultX、Loon、Shadowrocket:
1.将获取ck脚本存放到本地
2.打开云闪付，进入签到页面，若提示获取cookie成功则可以使用该脚本
            ...
            return new Promise(resolve => {
                $.post(options, async (error, response, data) => {
                    try {
                        ...
                        debug(result);
                        if (result?.signedIn) {
                            DoubleLog(`✅签到成功！已连续签到${result?.signInDays?.days}天`);
                        } else {
