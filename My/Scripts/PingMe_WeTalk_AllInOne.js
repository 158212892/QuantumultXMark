//2026/06/30
/*
@Name：PingMe & WeTalk 双合一自动化签到+视频奖励 (Env.js 多环境适配版)
@Author：怎么肥事 & yaowuliu

[rewrite_local]
^https:\/\/api\.(pingmeapp\.net|wetalkapp\.com)\/app\/queryBalanceAndBonus url script-request-header https://raw.githubusercontent.com/ZenmoFeiShi/Qx/refs/heads/main/PingMe_WeTalk_AllInOne.js

[task_local]
25 8,20 * * * https://raw.githubusercontent.com/ZenmoFeiShi/Qx/refs/heads/main/PingMe_WeTalk_AllInOne.js, tag=PingMe_WeTalk联合签到, enabled=true
...
const PLATFORMS = {
  pingme: {
    ...
    uaTemplate: (model, iosVer, scale, cfn, darwin) => `PingMe/1.0.0 (${model}; iOS ${iosVer}; Scale/${scale}) CFNetwork/${cfn} Darwin/${darwin}`,
    isDeregistered: (msg) => typeof msg === 'string' && msg.indexOf('已被注销') !== -1
  },
  wetalk: {
    name: 'WeTalk',
    host: 'api.wetalkapp.com',
    storeKey: 'wetalk_accounts_v1',
    uaTemplate: (model, iosVer, scale, cfn, darwin) => `WeTalk/30.6.0 (com.innovationworks.wetalk; build:28; iOS ${iosVer}) Alamofire/5.4.3`,
...
function fingerprintOf(paramsRaw, isWeTalk) {
  if (isWeTalk) {
  ...
  return (isWeTalk ? 'fp_' : '') + MD5(base).slice(0, 12);
}

function migrateWeTalkStore(store) {
...
function loadStore(platKey) {
  const cfg = PLATFORMS[platKey];
  const raw = $.getdata(cfg.storeKey);
  const def = { version: platKey === 'wetalk' ? 2 : 1, accounts: {}, order: [] };
  if (!raw) return def;
  try {
    const obj = JSON.parse(raw);
    if (!obj.accounts) obj.accounts = {};
    if (!Array.isArray(obj.order)) obj.order = Object.keys(obj.accounts);
    return platKey === 'wetalk' ? migrateWeTalkStore(obj) : obj;
  } catch (e) {
    return def;
  }
}

function saveStore(platKey, store) {
  $.setdata(JSON.stringify(store), PLATFORMS[platKey].storeKey);
}

function buildUA(platKey, baseUA, seed) {
  ...
  return PLATFORMS[platKey].uaTemplate(model, iosVer, scale, cfn, darwin);
}

function buildUrl(platKey, path, capture, overrideDeviceId) {
  const params = {};
  ...
  return `https://${PLATFORMS[platKey].host}/app/${path}?${qs}`;
}
...
function genFakeDeviceId(platKey) {
  const suffix = platKey === 'wetalk' ? 'WeTalkIOS' : 'PingMeIOS';
  return `${randHex(8)}-${randHex(4)}-${randHex(4)}-${randHex(4)}-${randHex(12)}${suffix}`;
}

function buildHeaders(platKey, capture, ua) {
  ...
  headers['Host'] = PLATFORMS[platKey].host;
...
// ==================== 核心任务流 ====================
function runAccount(platKey, acc, index, total) {
  const cfg = PLATFORMS[platKey];
  const isWeTalk = platKey === 'wetalk';
  const email = isWeTalk ? (acc.email || emailKeyOf(acc.capture?.paramsRaw)) : (acc.email || safeDecode(acc.capture?.paramsRaw?.email));
  const tag = `[账号${index+1}/${total} ${acc.alias || email || acc.id}]`;
  const ua = buildUA(platKey, acc.baseUA, acc.uaSeed);
  const headers = buildHeaders(platKey, acc.capture, ua);
  const fakeDeviceId = genFakeDeviceId(platKey);
  ...
  function fetchApi(path, useFakeId, retry = 3) {
    ...
    return new Promise((resolve, reject) => {
      $.http.get({ url: buildUrl(platKey, path, acc.capture, overrideId), headers }).then(res => {
        resolve(res);
      }).catch(err => {
        ...
        if (retry > 0 && /SSL|SSLSessionState|timeout|timed out|reset|connection|network|stream closed|closed|EOF/i.test(m)) {
          return sleep(isWeTalk ? 1200 : 1500).then(() => fetchApi(path, useFakeId, retry - 1).then(resolve).catch(reject));
        }
        reject(err);
      });
    });
  ...
  return fetchApi('queryBalanceAndBonus').then(res => {
    try {
      ...
      if (d.retcode === 0) msgs.push(`💰 余额：${d.result.balance} Coins`);
        ...
        if (cfg.isDeregistered(d.retmsg)) flag.deregistered = true;
      }
...
if (typeof $request !== 'undefined' && $request) {
  let platKey = '';
  if ($request.url.indexOf(PLATFORMS.pingme.host) !== -1) platKey = 'pingme';
  else if ($request.url.indexOf(PLATFORMS.wetalk.host) !== -1) platKey = 'wetalk';
  ...
  if (!platKey) {
    ...
    const isWeTalk = platKey === 'wetalk';
    const email = isWeTalk ? emailKeyOf(paramsRaw) : safeDecode(paramsRaw.email);
    const accId = fingerprintOf(paramsRaw, isWeTalk);

    if (isWeTalk && !email) {
      notify(`⚠️ ${cfg.name} 抓取失败`, '请求里未取到 email 参数，请登录后重试。');
      $.done({});
    } else {
      const store = loadStore(platKey);
      const existed = !!store.accounts[accId];
      const uaSeed = existed ? store.accounts[accId].uaSeed : store.order.length;
      const alias = existed ? store.accounts[accId].alias : (email ? email : `账号${store.order.length + 1}`);
      ...
      store.accounts[accId] = {
        ...
        createdAt: existed ? store.accounts[accId].createdAt : Date.now(),
        updatedAt: Date.now()
      };
      if (!existed) store.order.push(accId);
      saveStore(platKey, store);

      notify(existed ? `🔄 ${cfg.name} 参数已更新` : `✅ ${cfg.name} 新账号已入库`, `${alias}\n当前平台账号总数：${store.order.length}`);
      $.done({});
    }
  }
} else {
  let allChain = Promise.resolve();
  const summaryReports = [];

  Object.keys(PLATFORMS).forEach(platKey => {
    allChain = allChain.then(() => {
      const cfg = PLATFORMS[platKey];
      const store = loadStore(platKey);
      const ids = store.order.filter(id => store.accounts[id]);
      
      if (!ids.length) {
        console.log(`【${cfg.name}】没有检测到可用账号，跳过。`);
      ...
      const results = [];
      const deadIds = [];
      let platChain = Promise.resolve();

      ids.forEach((id, idx) => {
        platChain = platChain.then(() => runAccount(platKey, store.accounts[id], idx, total))
          .then(r => {
            results.push(r.text);
      ...
      return platChain.then(() => {
        let extra = '';
        if (deadIds.length) {
          const freshStore = loadStore(platKey);
          deadIds.forEach(did => {
            delete freshStore.accounts[did];
            const pos = freshStore.order.indexOf(did);
            if (pos !== -1) freshStore.order.splice(pos, 1);
          });
          saveStore(platKey, freshStore);
          extra = `\n———\n🗑 已自动清理注销账号 ${deadIds.length} 个`;
        }
        summaryReports.push(`【${cfg.name}平台】总计 ${total} 个账号：\n${results.join('\n———\n')}${extra}`);
      });
    });
  });
  ...
  allChain.then(() => {
    if (summaryReports.length === 0) {
      notify('⚠️ 任务未执行', '两个平台均未抓取到有效账号，请先在 App 内触发抓包。');
    } else {
      notify('🎉 双平台联合任务全部完成', summaryReports.join('\n\n====================\n\n'));
    }
    $.done();
  }).catch(err => {
    notify('❌ 联合任务异常结束', String(err));
    $.done();
  });
}
