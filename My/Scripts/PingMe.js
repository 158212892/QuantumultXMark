//2026/06/29
/*
@Name：PingMe 自动化签到+视频奖励 (Env.js 多环境适配版)
...
// 保留原版通知排版格式，改用 $.msg 统一推送
function notify(title, body) {
  console.log(`【${scriptName} 通知】${title}\n${body}`);
  $.msg(scriptName, title, body);
}
  ...
  function fetchApi(path, useFakeId) {
    ...
    const attempt = (n) => {
      return new Promise((resolve, reject) => {
        $.http.get({ url: buildUrl(path, acc.capture, overrideId), headers }).then(res => {
          resolve(res);
        }).catch(err => {
          ...
          if (n < 3 && /SSL|SSLSessionState|timeout|timed out|reset|connection|network|stream closed|closed|EOF/i.test(m || '')) {
            return sleep(1500).then(() => attempt(n + 1).then(resolve).catch(reject));
          }
          reject(err);
        });
      });
